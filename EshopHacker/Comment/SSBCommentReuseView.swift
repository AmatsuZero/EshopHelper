    ...
    var model: SSBCommentViewModel.Comment? {
        didSet {
            ...
            var width = (title as NSString)
                .boundingRect(with: CGSize(width: CGFloat.greatestFiniteMagnitude, height: 16),
...
class SSBCommentTableViewCell: UITableViewCell, Reusable {
    ...
    var model: SSBCommentViewModel.Comment? {
        didSet {
            ...
            var width = (title as NSString).boundingRect(with: CGSize(width: CGFloat.greatestFiniteMagnitude, height: 16),
                                                         options: .usesFontLeading,
                                                         attributes: attr, context: nil).width
            happyButton.setTitle(title, for: .normal)
            happyButton.snp.updateConstraints { make in
            ...
            width = (title as NSString)
                .boundingRect(with: CGSize(width: CGFloat.greatestFiniteMagnitude, height: 16),
                              options: .usesFontLeading, attributes: attr, context: nil).width
            praiseButton.setTitle(title, for: .normal)
            praiseButton.snp.updateConstraints { make in
                make.width.equalTo(width + padding)
            }
