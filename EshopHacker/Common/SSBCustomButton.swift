...
class SSBCustomButton: UIButton {
    ...
    override func layoutSubviews() {
        ...
        //content的实际size
        let contentSize = CGSize(width: bounds.width - contentEdgeInsets.horizontal,
                                 height: bounds.height - contentEdgeInsets.vertical)
        //垂直布局
        if buttonImagePosition == .top || buttonImagePosition == .bottom {
           adjustVertical(contentSize: contentSize)
        } else if buttonImagePosition == .right {// 水平布局
            adjustRight(contentSize: contentSize)
        }
    }
    func adjustVertical(contentSize: CGSize) {
    ...
    func adjustRight(contentSize: CGSize) {
    ...
    override func sizeThatFits(_ size: CGSize) -> CGSize {
        ...
        let contentLimitSize = CGSize(width: size.width - contentEdgeInsets.horizontal,
                                      height: size.height - contentEdgeInsets.vertical)
        ...
        case .left, .right:
            ...
            let imageSize = imageView?.sizeThatFits(.init(width: .greatestFiniteMagnitude,
            ...
            resultSize.height += max(imageEdgeInsets.vertical + (imageSize?.height ?? 0),
                                     titleEdgeInsets.vertical + titleSize.height)
        }
        return resultSize
    }
