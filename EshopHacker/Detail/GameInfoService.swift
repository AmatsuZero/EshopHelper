...
struct SSBGameInfoViewModel: SSBViewModelProtocol {
    ...
    struct HeadData {
        ...
        init(_ game: T.Game, commentCount: Int) {
            ...
            developer = NSAttributedString(string: "\(game.pubDate)/\(game.publisher)/\(game.developer ?? "未知")", attributes: [
            ...
            if let label = game.recommendLabel,
                ...
                let recommendRate = game.recommendRate {
            ...
            if let modes = game.playMode {
                playMode += modes.map { mode in
                    let view = UIView()
    ...
    struct PriceData {
        ...
        let prices: [T.GamePrice]
        fileprivate(set) var hasMore = false
        
        init(lowestPrice: String?, prices: [T.GamePrice]) {
